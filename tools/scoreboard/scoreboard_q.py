...
from call_catalogue import anyCatalogue # for the function 'anyCatalogue' used below
...
def calculate_score(df, column_name, ref_Point, y, ID):
    ...
    if ref_Point=='avg' or ref_Point=='':
        ...
        Exception("No valid Reference point defined for indciator" + ID)
            
    score_std=(((df[column_name]-score_mean)**2).sum()/(df[column_name].count()-1))**0.5
...
def getScoreboardData(cat):
    print('Data download...')
    data=anyCatalogue(cat, 'JER Scoreboard_quarterly', lite=True)
...
def setLastYear(dataset):
    for indic in codes:
       setField(indic,'lastYear',lastyear)
    return dataset
...
# extract data
JER_Scoreboard = getScoreboardData(JER_catalogue_q)

JER_Scoreboard[['yearn','yearq']]=JER_Scoreboard.year.apply(lambda x: pd.Series(str(x).split("Q")))
JER_Scoreboard[['yearn','yearq']] = JER_Scoreboard[['yearn','yearq']].apply(pd.to_numeric)
JER_Scoreboard=JER_Scoreboard[JER_Scoreboard.yearq==quarter]
JER_Scoreboard=JER_Scoreboard[JER_Scoreboard.yearn>=2005]
JER_Scoreboard=JER_Scoreboard[['IND_CODE', 'Indicator', 'type', 'Order', 'lastYear', 'sense', 'change',
       'Group', 'geo', 'yearn','value_n', 'flag']]
JER_Scoreboard.columns=['IND_CODE', 'Indicator', 'type', 'Order', 'lastYear', 'sense', 'change',
       'Group', 'geo', 'year','value_n', 'flag']
...
for indic in set(JER_Scoreboard_b['IND_CODE']):
    i+=1
    print('Producing data for the last three years for indicator '+str(i)+' out of 27 - '+indic+'...')
...
#pivothf=pd.pivot_table(headline,values='levelflag',index=['rank','geo'],columns=['Order','Indicator','year'])
...
print('Saving data for comparison 2008 - 2020 - last available...')
...
pivotc.to_excel(writer,'Comparison 2008-2020')
